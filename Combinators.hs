...
tagParser = 
  (Tag <$> (char '<' *> many1 (satisfy (\c -> isAsciiLower c || isAsciiUpper c || isDigit c || c == '-')) <* char '>')) 
  <|> (ClosingTag <$> (string "</" *> many1 (satisfy (\c -> isAsciiLower c || isAsciiUpper c || isDigit c || c == '-')) <* char '>'))
